...
entity interface_fifo is
    ...
    Port (        
        ...
        num_elem: out std_logic_vector(W-1 downto 0)
    );
end interface_fifo;
...
signal num_elem_internal : std_logic_vector(W-1 downto 0);
...
begin
    ...
    begin
        if (reset = '1') then
            ...
            if(num_elem_internal = x"00") then
                empty <= '1';
            else
                empty <= '0';
            end if;
            if(num_elem_internal = x"FF") then
                full <= '1';
            else
                full <= '0';
            end if;
